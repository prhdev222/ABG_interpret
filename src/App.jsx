...
import { AlertTriangle, Activity, Wind, Calculator, Stethoscope, BookOpen, LibraryBig, CheckCircle2, Info } from "lucide-react";
...
function FormulaCard({ title, value, formula, substitution, result, reference, citation }) {
  return (
    <div className="rounded-2xl border bg-white p-4 space-y-2">
      <div className="text-sm font-semibold text-slate-900">{title}</div>
      <div className="text-2xl font-bold">{value}</div>
      <div className="text-xs text-slate-500">สูตร: {formula}</div>
      <div className="text-xs text-slate-600">แทนค่า: {substitution}</div>
      <div className="text-xs text-slate-700">ผลลัพธ์: {result}</div>
      <div className="text-xs text-slate-500">หมายเหตุ: {reference}</div>
      {citation ? <div className="text-[11px] text-slate-400">อ้างอิง: {citation}</div> : null}
    </div>
  );
}
...
function WeaningCriteriaCard({ title, passed, reason }) {
  return (
    <div className={`rounded-2xl border p-4 ${passed ? "bg-green-50 border-green-200" : "bg-amber-50 border-amber-200"}`}>
      <div className="flex items-center justify-between gap-3">
        <div className="font-semibold">{title}</div>
        <Badge variant="secondary">{passed ? "ผ่าน" : "ยังไม่ผ่าน"}</Badge>
      </div>
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <button type="button" className="inline-flex items-center gap-1 text-slate-500 hover:text-slate-700 align-middle">
...
export default function ABGVentInterpreterApp() {
  ...
  const referenceList = [
    {
      id: 1,
      title: "American Thoracic Society. Interpretation of Arterial Blood Gases (ABGs).",
      note: "ใช้เป็นฐานสำหรับหลักการแปล acid-base disorder และ expected compensation เบื้องต้น",
    },
    {
      id: 2,
      title: "Merck Manual Professional Edition. Acid-Base Regulation and Disorders.",
      note: "ใช้อ้างอิงกรอบคิดเรื่อง metabolic acidosis, anion gap, mixed disorder และการประเมิน compensation",
    },
    {
      id: 3,
      title: "Kraut JA, Madias NE. Serum anion gap: its uses and limitations in clinical medicine.",
      note: "ใช้อ้างอิงแนวคิด anion gap และ albumin correction",
    },
    {
      id: 4,
      title: "StatPearls. Physiology, Alveolar Gas Equation.",
      note: "ใช้อ้างอิง alveolar gas equation และหลักการ A–a gradient",
    },
    {
      id: 5,
      title: "The Acute Respiratory Distress Syndrome Network. Ventilation with Lower Tidal Volumes as Compared with Traditional Tidal Volumes for Acute Lung Injury and ARDS. N Engl J Med. 2000;342:1301–1308.",
      note: "ใช้อ้างอิงแนวคิด lung-protective ventilation, tidal volume ตาม predicted body weight และ plateau pressure threshold",
    },
    {
      id: 6,
      title: "MacIntyre NR, et al. Evidence-based guidelines for weaning and discontinuing ventilatory support.",
      note: "ใช้อ้างอิงแนวคิด readiness, SBT และ weaning pathway",
    },
  ];
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="rounded-3xl bg-white p-6 shadow-sm border">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold">ABG + Ventilator Interpreter</h1>
              <p className="mt-2 text-sm md:text-base text-slate-600">
                โปรแกรมต้นแบบสำหรับช่วยแปลผล ABG, แนะนำการดู waveform, ให้ข้อเสนอแนะเรื่อง ventilator และมี interactive pathway สำหรับ weaning/extubation
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <Badge variant="secondary">Thai UI</Badge>
                <Badge variant="secondary">ABG logic</Badge>
                <Badge variant="secondary">Vent suggestion</Badge>
                <Badge variant="secondary">Interactive Weaning</Badge>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <Card className="xl:col-span-1 rounded-3xl shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-xl"><Activity className="h-5 w-5" /> ค่า ABG / เคมีพื้นฐาน</CardTitle>
            </CardHeader>
          ...
          <Card className="xl:col-span-1 rounded-3xl shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-xl"><Wind className="h-5 w-5" /> Ventilator ปัจจุบัน</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2 col-span-2">
                <Label>Clinical note / context</Label>
                <Textarea value={data.note} onChange={(e) => setField("note", e.target.value)} className="min-h-24" />
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-6 xl:col-span-1">
            <Card className="rounded-3xl shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-xl"><Calculator className="h-5 w-5" /> สรุปการแปลผล</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="rounded-2xl border bg-slate-50 p-4">
                  <div className="font-semibold">Primary interpretation</div>
                  <div className="mt-2 text-sm text-slate-700">{abg.primary}</div>
                </div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <FormulaCard title="P/F ratio" value={format1(pfRatio)} formula={abg.formulas?.pfRatio?.formula ?? "P/F ratio = PaO₂ / FiO₂"} substitution={abg.formulas?.pfRatio?.substitution ?? "-"} result={abg.formulas?.pfRatio?.result ?? "-"} reference={abg.formulas?.pfRatio?.reference ?? "ใช้ประเมิน oxygenation defect"} citation="[4], [5]" />
                  <FormulaCard title="A–a gradient" value={format1(aagrad)} formula={abg.formulas?.aAGradient?.formula ?? "A–a gradient = PAO₂ − PaO₂"} substitution={abg.formulas?.aAGradient?.substitution ?? "-"} result={abg.formulas?.aAGradient?.result ?? "-"} reference={abg.formulas?.aAGradient?.reference ?? "ใช้ดู V/Q mismatch หรือ shunt"} citation="[4]" />
                  <FormulaCard title="Anion gap" value={format1(abg.calculations?.ag)} formula={abg.formulas?.ag?.formula ?? "AG = Na⁺ − (Cl⁻ + HCO₃⁻)"} substitution={abg.formulas?.ag?.substitution ?? "-"} result={abg.formulas?.ag?.result ?? "-"} reference={abg.formulas?.ag?.reference ?? "ใช้แยกชนิด metabolic acidosis"} citation="[1], [2], [3]" />
                  <FormulaCard title="Corrected AG" value={format1(abg.calculations?.agCorrected)} formula={abg.formulas?.agCorrected?.formula ?? "Corrected AG = AG + 2.5 × (4 − albumin)"} substitution={abg.formulas?.agCorrected?.substitution ?? "-"} result={abg.formulas?.agCorrected?.result ?? "-"} reference={abg.formulas?.agCorrected?.reference ?? "ใช้เมื่อ albumin ต่ำ"} citation="[2], [3]" />
                  <FormulaCard title="Delta ratio" value={format1(deltaRatio)} formula={abg.formulas?.deltaRatio?.formula ?? "Delta ratio = (Corrected AG − 12) / (24 − HCO₃⁻)"} substitution={abg.formulas?.deltaRatio?.substitution ?? "-"} result={abg.formulas?.deltaRatio?.result ?? "-"} reference={abg.formulas?.deltaRatio?.reference ?? "ช่วยหา mixed disorder"} citation="[1], [2]" />
                  <div className="rounded-2xl border p-4 bg-white space-y-2">
                    <div className="text-sm font-semibold text-slate-900">VT/IBW</div>
                    <div className="text-2xl font-bold">{format1(vtPerKg)}</div>
                    <div className="text-xs text-slate-500">สูตร: VT/IBW = Tidal volume (mL) / IBW (kg)</div>
                    <div className="text-xs text-slate-600">แทนค่า: = {format1(parsed.vt)} / {format1(parsed.ibw)}</div>
                    <div className="text-xs text-slate-700">ผลลัพธ์: VT/IBW = {format1(vtPerKg)} mL/kg</div>
                    <div className="text-xs text-slate-500">หมายเหตุ: ใช้ช่วยดู lung-protective ventilation</div>
                    <div className="text-[11px] text-slate-400">อ้างอิง: [5]</div>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-3xl shadow-sm">
              <CardHeader><CardTitle className="text-xl">ABG details</CardTitle></CardHeader>
              <CardContent className="space-y-2 text-sm text-slate-700">
                {abg.details.length ? abg.details.map((x, i) => <div key={i} className="rounded-xl border p-3">• {x}</div>) : <div className="rounded-xl border p-3 text-slate-500">ยังไม่มีรายละเอียดเพิ่มเติม</div>}
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="rounded-3xl shadow-sm">
            <CardHeader><CardTitle className="flex items-center gap-2 text-xl"><Wind className="h-5 w-5" /> ข้อเสนอแนะเรื่อง ventilator</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm text-slate-700">
              {vent.items.length ? vent.items.map((x, i) => <div key={i} className="rounded-xl border p-3">• {x}</div>) : <div className="rounded-xl border p-3 text-slate-500">กรอกข้อมูลเพิ่มเพื่อรับคำแนะนำ</div>}
            </CardContent>
          </Card>

          <Card className="rounded-3xl shadow-sm border-amber-200">
            <CardHeader><CardTitle className="flex items-center gap-2 text-xl"><AlertTriangle className="h-5 w-5" /> คำเตือนสำคัญ</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm text-slate-700">
              {[...abg.warnings, ...vent.cautions].length ? [...abg.warnings, ...vent.cautions].map((x, i) => <div key={i} className="rounded-xl border border-amber-200 bg-amber-50 p-3">• {x}</div>) : <div className="rounded-xl border p-3 text-slate-500">ยังไม่มีคำเตือนเด่นจากข้อมูลที่กรอก</div>}
              <div className="rounded-xl border border-red-200 bg-red-50 p-3">เครื่องมือนี้เป็นตัวช่วยคิดเบื้องต้น ไม่ควรใช้แทน clinical judgment, bedside assessment, waveform review, imaging และการประเมินผู้ป่วยจริง</div>
            </CardContent>
          </Card>
        </div>

        <Card className="rounded-3xl shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-xl"><BookOpen className="h-5 w-5" /> How it calculates / สูตรที่ใช้</CardTitle></CardHeader>
          <CardContent className="space-y-3 text-sm text-slate-700">
            <div className="rounded-2xl border bg-slate-50 p-4">โปรแกรมจะแสดงสูตรที่ใช้จริงใต้ค่าที่คำนวณแต่ละช่อง โดยเน้นสูตรมาตรฐานที่ใช้บ่อยในการแปล ABG เช่น P/F ratio, anion gap, corrected AG, delta ratio, Winter’s formula, expected compensation และ alveolar gas equation</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {formulaList.length ? formulaList.map((item, idx) => (
                <div key={idx} className="rounded-2xl border p-4 bg-white space-y-2">
                  <div className="font-semibold">{item.label}</div>
                  <div className="text-xs text-slate-500">สูตร: {item.formula}</div>
                  <div className="text-xs text-slate-600">แทนค่า: {item.substitution}</div>
                  <div className="text-xs text-slate-700">ผลลัพธ์: {item.result}</div>
                  <div className="text-xs text-slate-500">หมายเหตุ: {item.reference}</div>
                </div>
              )) : <div className="rounded-2xl border p-4 text-slate-500">กรอกข้อมูล ABG เพิ่ม แล้วส่วนนี้จะแสดงสูตรพร้อมการแทนค่าจริงโดยอัตโนมัติ</div>}
            </div>
          </CardContent>
        </Card>

        <Card className="rounded-3xl shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-xl"><Activity className="h-5 w-5" /> ควรไปดู graph อะไรต่อ (Expected Waveform Review)</CardTitle></CardHeader>
          <CardContent className="space-y-3 text-sm text-slate-700">
            {waveGuide.map((item, idx) => (
              <div key={idx} className="rounded-2xl border bg-white p-4 space-y-3">
                <div className="font-semibold text-slate-900">{item.title}</div>
                <div>
                  <div className="text-xs font-medium text-slate-500">ควรดู</div>
                  <div className="mt-1 space-y-1">{item.lookFor.map((x, i) => <div key={i}>• {x}</div>)}</div>
                </div>
                <div>
                  <div className="text-xs font-medium text-slate-500">ตีความได้ว่า</div>
                  <div className="mt-1 space-y-1">{item.meaning.map((x, i) => <div key={i}>• {x}</div>)}</div>
                </div>
                <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs text-slate-700">แนวทางถัดไป: {item.action}</div>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="rounded-3xl shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-xl"><Stethoscope className="h-5 w-5" /> Initial Ventilator Settings (Practical Guide)</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-slate-700">
            <div className="rounded-2xl border p-4 bg-white space-y-2">
              <div className="font-semibold">VCV</div>
        ...
        <Card className="rounded-3xl shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-xl"><CheckCircle2 className="h-5 w-5" /> Interactive Weaning / Extubation Pathway</CardTitle></CardHeader>
          <CardContent className="space-y-5 text-sm text-slate-700">
            <div className="rounded-2xl border bg-slate-50 p-4 space-y-3">
              <div>ใช้เป็น clinical pathway สำหรับคัดกรองว่าควรเริ่ม SBT หรือยัง และข้อมูลสนับสนุนการพิจารณาถอดท่อมีมากน้อยเพียงใด</div>
              <div className="flex flex-wrap gap-3">
                <InlineTooltip
                  label="SBT คืออะไร"
                  content="SBT หรือ Spontaneous Breathing Trial คือการทดลองให้ผู้ป่วยหายใจเองด้วยการช่วยจากเครื่องในระดับต่ำชั่วคราว เช่น PSV 5–7 ร่วมกับ PEEP 5 หรือ T-piece เพื่อดูว่าผู้ป่วยทนการหายใจเองได้หรือไม่ก่อนพิจารณาถอดท่อ"
                />
                <InlineTooltip
                  label="RSBI คืออะไร"
                  content="RSBI หรือ Rapid Shallow Breathing Index คำนวณจาก RR / VT(ลิตร) ใช้เป็นตัวช่วยทำนายโอกาส weaning สำเร็จ โดยทั่วไปค่า < 105 สนับสนุนความเป็นไปได้ที่จะหย่าเครื่องได้สำเร็จ แต่ไม่ใช้แทน clinical judgment"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                ...
                <div key={key} className="rounded-2xl border bg-white p-4 space-y-2">
                  <Label>{label}</Label>
            ...
            <div className={`rounded-2xl border p-4 ${overallWeaningReady ? "bg-green-50 border-green-200" : "bg-amber-50 border-amber-200"}`}>
              <div className="flex items-center gap-2 font-semibold">Step 1: SBT readiness <InlineTooltip label="อธิบาย" content="ประเมินว่าผู้ป่วยพร้อมเข้าสู่การทดลองหายใจเองหรือยัง โดยดู clinical readiness ร่วมกับ oxygenation และ ventilation readiness ก่อนเริ่ม SBT" /></div>
              <div className="mt-2">{clinicallyReady ? "Clinical readiness ผ่าน" : "Clinical readiness ยังไม่ผ่าน"}</div>
              <div className="mt-1">{weaningAssessment.pathway}</div>
            </div>

            <div className={`rounded-2xl border p-4 ${extubationSupportive ? "bg-green-50 border-green-200" : "bg-slate-50"}`}>
              <div className="flex items-center gap-2 font-semibold">Step 2: หลังผ่าน SBT ให้ดู extubation support <InlineTooltip label="อธิบาย" content="แม้ผ่าน SBT แล้ว ยังต้องประเมินต่อว่าสามารถถอดท่อได้อย่างปลอดภัยหรือไม่ โดยดู airway protection, cough, secretion burden, mental status และดัชนีสนับสนุน เช่น RSBI" /></div>
              <div className="mt-2 flex items-center gap-2">RSBI = {format1(weaningAssessment.rsbi)} <InlineTooltip label="สูตร" content="RSBI = RR / VT(ลิตร) เช่น RR 30 ครั้ง/นาที และ VT 0.3 ลิตร จะได้ RSBI = 100 โดยทั่วไปค่า < 105 สนับสนุนโอกาส weaning สำเร็จ" /></div>
              <div className="mt-1">{extubationSupportive ? "ข้อมูลปัจจุบันสนับสนุนการพิจารณาถอดท่อ หาก airway protection/cough/secretion เหมาะสม" : "ยังควรประเมินเพิ่มก่อนตัดสินใจ extubate"}</div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="rounded-2xl border bg-white p-4 space-y-2">
                <div className="flex items-center gap-2 font-semibold">SBT practical guide <InlineTooltip label="หลักการ" content="SBT มักทำ 30–120 นาที โดยใช้การช่วยต่ำ เช่น PSV 5–7 กับ PEEP 5 หรือ T-piece แล้วดูว่าผู้ป่วยทนได้หรือไม่ หากมี tachypnea, desaturation, hemodynamic instability หรือ distress ให้ถือว่าล้มเหลว" /></div>
                {weaningAssessment.sbtAdvice.map((x, i) => <div key={i}>• {x}</div>)}
              </div>
              <div className="rounded-2xl border bg-white p-4 space-y-2">
                <div className="flex items-center gap-2 font-semibold">Extubation checklist <InlineTooltip label="หลักการ" content="การถอดท่อไม่ดูแค่ผ่าน SBT ต้องมี airway protection, cough ดี, secretion ไม่มาก, mental status พอ, และบริบททางคลินิกเอื้อด้วย จึงจะลดโอกาส reintubation" /></div>
                {weaningAssessment.extubationChecklist.map((x, i) => <div key={i}>• {x}</div>)}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-3xl shadow-sm">
          <CardHeader><CardTitle className="flex items-center gap-2 text-xl"><LibraryBig className="h-5 w-5" /> References / เอกสารอ้างอิง</CardTitle></CardHeader>
          <CardContent className="space-y-3 text-sm text-slate-700">
            <div className="rounded-2xl border bg-slate-50 p-4">รายการอ้างอิงด้านล่างใช้เป็นฐานของสูตรและกรอบการตีความในโปรแกรมนี้ รวมทั้งส่วน weaning pathway</div>
            <div className="space-y-3">
              {referenceList.map((ref) => (
                <div key={ref.id} className="rounded-2xl border bg-white p-4 space-y-1">
                  <div className="font-semibold">[{ref.id}] {ref.title}</div>
        ...
        <Card className="rounded-3xl shadow-sm">
          <CardHeader><CardTitle className="text-xl">สรุปพร้อมคัดลอก</CardTitle></CardHeader>
          <CardContent><Textarea value={summaryText} readOnly className="min-h-40" /></CardContent>
        </Card>
      </div>
    </div>
    </TooltipProvider>
